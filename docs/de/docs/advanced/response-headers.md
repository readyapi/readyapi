...
Sie können einen Parameter vom Typ `Response` in Ihrer _Pfadoperation-Funktion_ deklarieren (wie Sie es auch für Cookies tun können).

Und dann können Sie Header in diesem _vorübergehenden_ Response-Objekt festlegen.
...
**ReadyAPI** verwendet diese _vorübergehende_ Response, um die Header (auch Cookies und Statuscode) zu extrahieren und fügt diese in die endgültige Response ein, die den von Ihnen zurückgegebenen Wert enthält, gefiltert nach einem beliebigen `response_model`.
