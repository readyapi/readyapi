...
**Note**: You can mix `def` and `async def` in your _path operation functions_ as much as you need and define each one using the best option for you. ReadyAPI will do the right thing with them.
...
If you are working with **ReadyAPI** you don't have to worry about that, because that "first" function will be your _path operation function_, and ReadyAPI will know how to do the right thing.
...
And even if you were not using ReadyAPI, you could also write your own async applications with <a href="https://anyio.readthedocs.io/en/stable/" class="external-link" target="_blank">AnyIO</a> to be highly compatible and get its benefits (e.g. _structured concurrency_).
...
This is in contrast to the functions that ReadyAPI calls for you: _path operation functions_ and dependencies.
