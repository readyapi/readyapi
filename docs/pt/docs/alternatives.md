...
Na verdade é comum utilizar Requests _dentro_ de uma aplicação ReadyAPI.

Ainda assim, ReadyAPI pegou alguma inspiração do Requests.

**Requests** é uma biblioteca para interagir com APIs (como um cliente), enquanto **ReadyAPI** é uma biblioteca para _construir_ APIs (como um servidor).
...
A contra-parte da aplicação ReadyAPI, _rota de operação_, poderia parecer como:
