...
Eğer ne olduğunu bilmiyor isen _"Acelen mi var?"_ kısmını oku <a href="https://readyapi.khulnasoft.com/async/#in-a-hurry" target="_blank">`async` ve `await`</a>.
...
* `app`: ise `main.py` dosyasının içerisinde oluşturduğumuz `app = ReadyAPI()` 'a denk geliyor.
