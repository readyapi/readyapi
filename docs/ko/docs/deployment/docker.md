...
만약 여러분의 ReadyAPI가 하나의 파일이라면, 예를 들어 `./app` 디렉터리 없이 `main.py` 파일만으로 이루어져 있다면, 파일 구조는 다음과 유사할 것입니다:
...
그리고 일반적으로 **로드 밸런서**는 여러분의 클러스터에 있는 _다른_ 앱으로 가는 요청들도 다룰 수 있으며 (예를 들어, 다른 도메인으로 가거나 다른 URL 경로 접두사를 가지는 경우), 이 통신들을 클러스터에 있는 _바로 그 다른_ 어플리케이션으로 제대로 전송할 수 있습니다.
