...
**readyapi**는 그 *임시* 응답에서 쿠키(또한 헤더 및 상태 코드)를 추출하고, 반환된 값이 포함된 최종 응답에 이를 넣습니다. 이 값은 `response_model`로 걸러지게 됩니다.
...
`Response` 매개변수를 사용하지 않고 응답을 직접 반환하는 경우, readyapi는 이를 직접 반환한다는 점에 유의하세요.
...
**readyapi**는 개발자의 편의를 위해 `readyapi.responses`로 동일한 `starlette.responses`를 제공합니다. 그러나 대부분의 응답은 Starlette에서 직접 제공됩니다.

또한 `Response`는 헤더와 쿠키를 설정하는 데 자주 사용되므로, **readyapi**는 이를 `readyapi.Response`로도 제공합니다.
