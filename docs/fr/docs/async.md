...
**Note** : vous pouvez mélanger `def` et `async def` dans vos _fonctions de chemin_ autant que nécessaire, **ReadyAPI** saura faire ce qu'il faut avec.
...
Si vous utilisez **ReadyAPI**, pas besoin de vous en inquiéter, car cette "première" fonction sera votre _fonction de chemin_ ; et **ReadyAPI** saura comment arriver au résultat attendu.
...
Contrairement aux fonctions que **ReadyAPI** appelle pour vous : les _fonctions de chemin_ et dépendances.
