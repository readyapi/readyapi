...
如里不了解异步，请参阅[异步：_“着急了？”_](../../async.md){.internal-link target=\_blank} 一章中 `async` 和 `await` 的内容。
...
通过依赖注入系统，只要告诉 **ReadyAPI** _路径操作函数_ 还要「依赖」其他在*路径操作函数*之前执行的内容，**ReadyAPI** 就会执行函数代码，并「注入」函数返回的结果。
