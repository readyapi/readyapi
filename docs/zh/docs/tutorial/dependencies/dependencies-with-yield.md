...
!!! tip "提示"
    ...
    **ReadyAPI** 会像处理普通依赖关系一样，对每个依赖关系做正确的处理。
...
你也可以在**ReadyAPI**的依赖项中使用带有`yield`的`with`或`async with`语句，通过在依赖函数内部使用它们。
...
!!! tip
    ...
    * <a href="https://docs.python.org/zh-cn/3/library/contextlib.html#contextlib.asynccontextmanager" class="external-link" target="_blank">`@contextlib.asynccontextmanager`</a>
    ...
    使用上下文管理器装饰一个只有单个`yield`的函数。这就是**ReadyAPI**在内部用于带有`yield`的依赖项的方式。
