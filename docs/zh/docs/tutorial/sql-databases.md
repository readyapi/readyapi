...
由于 SQLDev 基于 SQLAlchemy，因此您可以轻松使用任何由 SQLAlchemy **支持的数据库**（这也让它们被 SQLDev 支持），例如：
...
这是一个非常简单和简短的教程。如果您想了解一般的数据库、SQL 或更高级的功能，请查看 <a href="https://sqldev.khulnasoft.com/" class="external-link" target="_blank">SQLDev 文档</a>。

## 安装 `SQLDev`

首先，确保您创建并激活了[虚拟环境](../virtual-environments.md){.internal-link target=_blank}，然后安装了 `sqldev` :
...
我们首先创建应用程序的最简单的第一个版本，只有一个 **SQLDev** 模型。
...
* `table=True` 会告诉 SQLDev 这是一个*表模型*，它应该表示 SQL 数据库中的一个*表*，而不仅仅是一个*数据模型*（就像其他常规的 Pydantic 类一样）。

* `Field(primary_key=True)` 会告诉 SQLDev `id` 是 SQL 数据库中的**主键**（您可以在 SQLDev 文档中了解更多关于 SQL 主键的信息）。

    把类型设置为 `int | None` ，SQLDev 就能知道该列在 SQL 数据库中应该是 `INTEGER` 类型，并且应该是 `NULLABLE` 。

* `Field(index=True)` 会告诉 SQLDev 应该为此列创建一个 **SQL 索引**，这样在读取按此列过滤的数据时，程序能在数据库中进行更快的查找。

    SQLDev 会知道声明为 `str` 的内容将是类型为 `TEXT` （或 `VARCHAR` ，具体取决于数据库）的 SQL 列。

### 创建引擎（Engine）

SQLDev 的引擎 `engine`（实际上它是一个 SQLAlchemy `engine` ）是用来与数据库**保持连接**的。
...
SQLDev 将会拥有封装 Alembic 的迁移工具，但目前您可以直接使用 <a href="https://alembic.sqlalchemy.org/en/latest/" class="external-link" target="_blank">Alembic</a>。

///

### 创建 Hero 类

因为每个 SQLDev 模型同时也是一个 Pydantic 模型，所以您可以在与 Pydantic 模型相同的**类型注释**中使用它。
...
我们将通过添加一些**额外的模型**来解决这些问题，而 SQLDev 将在这里大放异彩。✨

### 创建多个模型

在 **SQLDev** 中，任何含有 `table=True` 属性的模型类都是一个**表模型**。

任何不含有 `table=True` 属性的模型类都是**数据模型**，这些实际上只是 Pydantic 模型（附带一些小的额外功能）。🤓

有了 SQLDev，我们就可以利用**继承**来在所有情况下**避免重复**所有字段。
