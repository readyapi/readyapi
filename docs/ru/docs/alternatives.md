...
В принципе, можно использовать Requests _внутри_ приложения ReadyAPI.
...
в то время как **ReadyAPI** - это библиотека для _создания_ API (то есть сервера).
...
Противоположная _операция пути_ в ReadyAPI может выглядеть следующим образом:
