...
Вы можете создавать фоновые задачи, которые будут выполнятся _после_ возвращения ответа сервером.
...
Внутри функции вызовите метод `.add_task()` у объекта _background tasks_ и передайте ему функцию, которую хотите выполнить в фоне:
...
В этом примере сообщения будут записаны в `log.txt` _после_ того, как ответ сервера был отправлен.
