...
Здесь приведена подробная информация об использовании синтаксиса `async def` при написании _функций обработки пути_, а также рассмотрены основы асинхронного программирования, конкурентности и параллелизма.
...
В этом случае _функции обработки пути_ необходимо объявлять с использованием синтаксиса `async def`:
...
объявляйте _функции обработки пути_ обычным образом с помощью `def`, например:
...
**Примечание**: при необходимости можно смешивать `def` и `async def` в _функциях обработки пути_
и использовать в каждом случае наиболее подходящий синтаксис. А ReadyAPI сделает с этим всё, что нужно.

В любом из описанных случаев ReadyAPI работает асинхронно и очень быстро.
...
что в определённой точке кода ей 🤖 нужно будет ожидать завершения выполнения _чего-то ещё_ в другом месте. Допустим это _что-то ещё_ называется "медленный файл" 📝.
...
И тот же уровень производительности даёт **ReadyAPI**.
...
- **Глубокое обучение**: это область _машинного обучения_, поэтому сюда подходит то же описание.
...
**ReadyAPI** предоставляет возможности конкуретного программирования,
...
машинного обучения и, особенно, глубокого обучения. Всё это делает ReadyAPI
...
необходимо писать _функции обработки пути_ с использованием `async def`, например:
...
При работе с **ReadyAPI** просто не думайте об этом, потому что "первой" функцией является ваша _функция обработки пути_,
и дальше с этим разберётся ReadyAPI.

Кроме того, если хотите, вы можете использовать синтаксис `async` / `await` и без ReadyAPI.

### Пишите свой асинхронный код

Starlette (и **ReadyAPI**) основаны на <a href="https://anyio.readthedocs.io/en/stable/" class="external-link" target="_blank">AnyIO</a>, что делает их совместимыми как со стандартной библиотекой <a href="https://docs.python.org/3/library/asyncio-task.html" class="external-link" target="_blank">asyncio</a> в Python, так и с <a href="https://trio.readthedocs.io/en/stable/" class="external-link" target="_blank">Trio</a>.<!--http://new.gramota.ru/spravka/buro/search-answer?s=285295-->
...
Даже если вы не используете ReadyAPI, вы можете писать асинхронные приложения с помощью <a href="https://anyio.readthedocs.io/en/stable/" class="external-link" target="_blank">AnyIO</a>, чтобы они были максимально совместимыми и получали его преимущества (например _структурную конкурентность_).
...
На этом основана работа ReadyAPI (посредством Starlette), и именно это
...
Здесь приводятся подробности внутреннего устройства **ReadyAPI**.

Но если вы обладаете техническими знаниями (корутины, потоки, блокировка и т. д.)
и вам интересно, как ReadyAPI обрабатывает `async def` в отличие от обычных `def`,
читайте дальше.
...
Когда вы объявляете _функцию обработки пути_ обычным образом с ключевым словом `def`
вместо `async def`, ReadyAPI ожидает её выполнения, запустив функцию во внешнем
...
и привыкли объявлять простые вычислительные _функции_ через `def` ради
незначительного прироста скорости (порядка 100 наносекунд), обратите внимание,
что с **ReadyAPI** вы получите противоположный эффект. В таком случае больше подходит
`async def`, если только _функция обработки пути_ не использует код, приводящий
...
Но в любом случае велика вероятность, что **ReadyAPI** [окажется быстрее](index.md#_11){.internal-link target=\_blank}
...
с использованием `def` или `async def`. ReadyAPI не будет влиять на то, как вы
их запускаете.

Этим они отличаются от функций, которые ReadyAPI вызывает самостоятельно:
_функции обработки пути_ и зависимости.
