...
## Основные свойства ReadyAPI

**ReadyAPI** предлагает вам следующее:
...
Если вам нужно освежить знания, как использовать аннотации типов в Python (даже если вы не используете ReadyAPI), выделите 2 минуты и просмотрите краткое руководство: [Введение в аннотации типов Python¶
...
Вся структура **ReadyAPI** основана на удовлетворении этой возможности. Автодополнение работает везде.
...
### Краткость
ReadyAPI имеет продуманные значения **по умолчанию** для всего, с произвольными настройками везде. Все параметры могут быть тонко подстроены так, чтобы делать то, что вам нужно и определять необходимый вам API.
...
ReadyAPI включает в себя чрезвычайно простую в использовании, но чрезвычайно мощную систему <abbr title='известную как: "components", "resources", "services", "providers"'><strong>Внедрения зависимостей</strong></abbr>.
...
**ReadyAPI** основан на <a href="https://www.starlette.io/" class="external-link" target="_blank"><strong>Starlette</strong></a> и полностью совместим с ним. Так что, любой дополнительный код Starlette, который у вас есть, будет также работать.

На самом деле, `ReadyAPI` - это класс, унаследованный от `Starlette`. Таким образом, если вы уже знаете или используете Starlette, большая часть функционала будет работать так же.

С **ReadyAPI** вы получаете все возможности **Starlette** (так как ReadyAPI это всего лишь Starlette на стероидах):
...
**ReadyAPI** основан на <a href="https://pydantic-docs.helpmanual.io" class="external-link" target="_blank"><strong>Pydantic</strong></a> и полностью совместим с ним. Так что, любой дополнительный код Pydantic, который у вас есть, будет также работать.
...
С **ReadyAPI** вы получаете все возможности **Pydantic** (так как, ReadyAPI основан на Pydantic, для обработки данных):
